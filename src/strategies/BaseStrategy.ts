import type { Card, Action, CasinoRules, StrategyProvider } from '../types';
import { getHandValue, getDealerUpCardValue, isPair, getCardValue } from '../utils/deck';

// Strategy lookup tables type
export type StrategyTable = Record<number, Record<number, Action>>;

export abstract class BaseStrategy implements StrategyProvider {
  abstract name: string;
  abstract description: string;
  abstract rules: CasinoRules;

  protected abstract hardStrategy: StrategyTable;
  protected abstract softStrategy: StrategyTable;
  protected abstract pairStrategy: StrategyTable;
  ...
  getAction(
    ...
    if (playerHand.length === 2 && isPair(playerHand) && canSplit) {
      ...
      const pairAction = this.pairStrategy[pairValue]?.[dealerValue];
    ...
    if (handValue.isSoft && handValue.value <= 21) {
      const softAction = this.softStrategy[handValue.value]?.[dealerValue];
      if (softAction) {
    ...
    const hardAction = this.hardStrategy[hardTotal]?.[dealerValue];
  ...
  getExplanation(
    ...
    // Generic fallback
    return `Basic strategy recommends ${this.getActionName(correctAction).toLowerCase()} with ${handValue.value} against dealer ${dealerCardName}.`;
  }
